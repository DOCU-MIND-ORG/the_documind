...
export default function Dashboard() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // 1. Derive a session title from the message
      const title = query.length <= 40 ? query : query.slice(0, 40) + '…';

      // 2. Create session
      const session = await sessionService.create(title);

      // 3. Add to sidebar immediately
      addSession(session);

      // 4. Navigate to chat, passing the original message as state
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <header className="flex items-center h-14 px-4 border-b border-white/[0.05] shrink-0">
        <button
          className="md:hidden p-2 -ml-1 mr-2 text-slate-400 hover:text-white rounded-xl hover:bg-white/[0.05] transition-all"
        ...
        <span className="text-[13px] font-medium text-slate-500">DocuMind</span>
      </header>
